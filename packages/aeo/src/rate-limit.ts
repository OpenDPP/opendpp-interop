 ...
 *
 * The EU EOS service caps each source at **100 requests/second** (and 10 search
 * criteria per request). This module is a tiny, zero-dependency rate limiter that
 * paces outgoing requests to that cap so the package is a good citizen against a
 * government endpoint, while staying fully overridable: the EU tech team grants
 * higher/uncapped limits on request, so the rate can be raised — or disabled —
 ...
 * scaled service a shared/server-side limiter is still needed; this protects a
 * single process from bursting past the cap.
 *
 * (Mirrors @opendpp/eori's limiter; kept local so the package stays zero-dependency
 ...
 */
export class AeoRateLimiter implements RateLimiter {
  #minIntervalMs = 0;
  #nextSlot = 0;
...
/**
 * Create a standalone limiter — e.g. one shared across the specific lookups that may
 * use a higher granted limit: `lookupAeo(q, { rateLimiter: createAeoRateLimiter(500) })`.
 ...
 */
export function createAeoRateLimiter(
  requestsPerSecond: number | null = DEFAULT_REQUESTS_PER_SECOND,
): AeoRateLimiter {
  return new AeoRateLimiter(requestsPerSecond);
}

/**
 * The process-wide default limiter used by `lookupAeo`/`lookupAeoBatch` when no
 * per-call `rateLimiter` is supplied. Reconfigured in place by `setDefaultAeoRateLimit`,
 * so this binding stays stable.
 */
export const defaultAeoRateLimiter = new AeoRateLimiter(DEFAULT_REQUESTS_PER_SECOND);

/**
 * Reconfigure the shared default limit (requests/second). `null`/`0` disables it.
 * One-liner for a process granted a higher limit: `setDefaultAeoRateLimit(500)`.
 */
export function setDefaultAeoRateLimit(requestsPerSecond: number | null): void {
  defaultAeoRateLimiter.setRate(requestsPerSecond);
}
