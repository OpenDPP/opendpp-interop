...
| `aas-test-engines` (IDTA gold standard) | ✅ | **all 9 per-category** AAS samples (`samples/*.aasx`) pass the official Python `aas-test-engines` validator — the AAS v3.0 metamodel gold standard — in OpenDPP's backend CI; the kit CI JSON-Schema-validates each (extracts `aasx/data/environment.json`). |
