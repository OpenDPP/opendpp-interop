...
| Renders in the CIRPASS-2 *reference* viewer (`dpp-renderer-be`) via OpenDPP JSON-LD | 🟡 Partial | The viewer's `GET /fetch/v1` Jena-parses + Titanium-JSON-LD-**expands** a fetched DPP. OpenDPP serves an inline-`@context` `application/ld+json` door whose **RDF-expandability — the property the viewer relies on — is proven in CI** by a hermetic guard: every variant (passport ×3 tiers, unit ×2, tombstone) expands with **zero OpenDPP-term drops** (backend `tests/functional/jsonld-expand.test.ts`, OpenDPP #174). The **one-time live** `dpp-renderer-be /fetch/v1` render is a documented **manual reproduction** (laptop-only, like the `aas-test-engines` gold standard) — OpenDPP #432; the captured expanded-RDF sample lands here when run. **Vocabulary stance: Option B** — OpenDPP stays on its own `opendpp-node.eu/ns/dpp#` vocab (the renderer's `unknownOntology` heuristic) and does **not** adopt the non-normative TalTech EUDPP vocab. **Viewer interop only** (the renderer ignores AAS/VC); *reference renderer*, **never** *certified*. |
